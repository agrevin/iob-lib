...
def setup(python_module, no_overlap=False, peripheral_ios=True, internal_wires=None):
    ...
    confs.append({'name': 'VERSION', 'type': 'M', 'val': "16'h"+build_srcs.version_str_to_digits(
        python_module.version), 'min': 'NA', 'max': 'NA', 'descr': "Product version."})
    ...
    peripherals_list = iob_soc_peripheral_setup(
        python_module, append_peripheral_ios=peripheral_ios)

    # Build periphs_tmp.h
    if peripherals_list:
        periphs_tmp.create_periphs_tmp(python_module.name.upper(),
            31, peripherals_list, f"{python_module.build_dir}/software/{top}_periphs.h")
    # Try to build iob_soc.v if template is available
    createSystem.create_systemv(python_module.setup_dir, python_module.submodules['dirs'], python_module.name, peripherals_list, os.path.join(
        python_module.build_dir, f'hardware/src/{top}.v'), internal_wires=internal_wires)
    # Try to build system_tb.v if template is available
    createTestbench.create_system_testbench(python_module.setup_dir, python_module.submodules['dirs'], python_module.name, peripherals_list, os.path.join(
        python_module.build_dir, f'hardware/simulation/src/{top}_tb.v'))
    # Try to build system_top.v if template is available
    createTopSystem.create_top_system(python_module.setup_dir, python_module.submodules['dirs'], python_module.name, peripherals_list, ios, confs, os.path.join(
        python_module.build_dir, f'hardware/simulation/src/{top}_top.v'))
    ...
    if regs:
        ...
        general_regs_table = next(
            (i for i in regs if i['name'] == 'general'), None)
        if not general_regs_table:
            general_regs_table = {'name': 'general',
                                  'descr': 'General Registers.', 'regs': []}
            regs.append(general_regs_table)
        # Auto add 'VERSION' register in 'general' registers table
        general_regs_table['regs'].append({'name': "VERSION", 'type': "R", 'n_bits': 16, 'rst_val': build_srcs.version_str_to_digits(
            python_module.version), 'addr': -1, 'log2n_items': 0, 'autologic': True, 'descr': "Product version."})
        ...
        if 'hw_setup' not in python_module.submodules:
            python_module.submodules['hw_setup'] = {
                'headers': [], 'modules': []}
    ...
    if regs:
        ...
        mkregs_obj.write_hwcode(reg_table, build_dir +
                                '/hardware/src', top, top_if)
    ...
    if regs:
        ...
        mkregs_obj.write_swheader(
            reg_table, python_module.build_dir+'/software/esrc', top)
        ...
        mkregs_obj.write_swheader(
            reg_table, python_module.build_dir+'/software/psrc', top)
